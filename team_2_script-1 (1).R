# the recording of the process of generating the final predictions can be accessed at:
# https://drive.google.com/file/d/1punhiKzF0ODWtHiXlEOVwHVcZu4czFt8/view
# load packages
...
scaled_recipe_41 = recipe_41 %>% 
  step_impute_median(all_predictors())%>%
  step_lincomb(all_predictors()) %>%
  step_normalize(all_numeric(), -all_outcomes()) 

# xgboost with all predictors: 5.983
xgb_spec <- boost_tree(
  trees = 958,
  min_n = 39,
  tree_depth = 10,
  learn_rate = 0.01596140,
  loss_reduction = 1.607620e-09
) %>% 
  set_engine("xgboost", lambda=1.810340e-05, objective = "reg:squarederror") %>%
  set_mode("regression")
# XGBoost workflow
xgb_wf <- workflow() %>% 
...
# xgboost with 41 predictors: 6.2
xgb_spec_41 <- boost_tree(
  trees = 995,
  min_n = 16,
  tree_depth = 3,
  learn_rate = 0.04442623,
  loss_reduction = 8.010154e-09
) %>% 
  set_engine("xgboost", objective = "reg:squarederror") %>%
  set_mode("regression")
# XGBoost workflow
xgb_wf_41 <- workflow() %>% 
  add_recipe(recipe_41) %>%
  add_model(xgb_spec_41)
...
nnet_wf_41 <- workflow() %>%
  add_recipe(scaled_recipe_41) %>%
  add_model(nnet_spec_41)

# create workflow set containing all model workflows
final_wf_set <- as_workflow_set(xgb = xgb_wf, 
                                xgb_41 = xgb_wf_41,
                                rf = rf_wf,
                                rf_41 = rf_wf_41,
                                knn = knn_wf,
...
# make predictions and save as csv 
test_results <- final_set_stacks %>% 
...
write_csv(test_results, 'team_2_results_stacks_final.csv')
